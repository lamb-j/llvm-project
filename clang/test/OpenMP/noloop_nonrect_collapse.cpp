...
// RUN: %clang_cc1 -fopenmp -fopenmp-target-fast -fno-openmp-target-big-jump-loop -fopenmp-assume-teams-oversubscription -fopenmp-assume-threads-oversubscription -x c++ -std=c++11 -fopenmp-version=50 -triple x86_64-unknown-unknown -fopenmp-targets=amdgcn-amd-amdhsa -emit-llvm-bc %s -o %t-ppc-host.bc
// RUN: %clang_cc1 -fopenmp -fopenmp-target-fast -fno-openmp-target-big-jump-loop -fopenmp-assume-teams-oversubscription -fopenmp-assume-threads-oversubscription -x c++ -std=c++11 -fopenmp-version=50 -triple amdgcn-amd-amdhsa -fopenmp-targets=amdgcn-amd-amdhsa -emit-llvm %s -fopenmp-is-device -fopenmp-host-ir-file-path %t-ppc-host.bc -o - | FileCheck %s
